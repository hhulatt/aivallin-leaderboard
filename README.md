# AivAllIn — Rainbet wager leaderboard
...
5. **Point the domain** at Netlify and wait for the TLS certificate. A new
   ...
   certificate is actually issued for `aivallin.com` before telling anyone the
   site is live.
...
`creators/aivallin.json` in the template repo. Edit that, re-run
`node new-creator.mjs aivallin`, and copy the regenerated files over. The cycle
